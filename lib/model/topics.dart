...
class Topics extends PerAccountStoreBase with ChangeNotifier {
  ...
  Future<List<GetChannelTopicsEntry>> getChannelTopics(int channelId) async {
    ...
    Future<List<GetChannelTopicsEntry>>? future =
        _channelTopicsFetching[channelId];
  ...
  Future<List<GetChannelTopicsEntry>> _getChannelTopics(int channelId) async {
    final result = await _apiGetChannelTopics(
      connection,
      channelId: channelId,
    ...
    (_latestMessageIdsByChannelTopic[channelId] = makeTopicKeyedMap())
        .addEntries(
          result.topics.map((entry) => MapEntry(entry.name, entry.maxId)),
        );
    notifyListeners();
  ...
  void handleMessageEvent(MessageEvent event) {
    ...
    final StreamMessage(:id, streamId: channelId, :topic) =
        event.message as StreamMessage;
  ...
  void handleUpdateMessageEvent(UpdateMessageEvent event) {
    ...
    final origLatestMessageIdsByTopic =
        _latestMessageIdsByChannelTopic[origStreamId];
    ...
    final newLatestMessageIdsByTopic =
        _latestMessageIdsByChannelTopic[newStreamId];
