...
void main() {
  ...
  Future<void> prepareComposeBox(
    ...
    if (narrow
        case ChannelNarrow(:var streamId) || TopicNarrow(:var streamId)) {
  ...
  Future<void> enterTopic(
    ...
    connection.prepare(
      body: jsonEncode(
        GetChannelTopicsResult(topics: [eg.getChannelTopicsEntry()]).toJson(),
      ),
    );
  ...
  void checkContentInputValue(WidgetTester tester, String expected) {
    check(
      tester.widget<TextField>(contentInputFinder),
  ...
  group('auto focus', () {
    ...
    testWidgets('ChannelNarrow, empty fetch', (tester) async {
      ...
      check(
        controller,
      ).isA<StreamComposeBoxController>().topicFocusNode.hasFocus.isTrue();
    });
    ...
    testWidgets('DmNarrow, non-empty fetch', (tester) async {
      ...
      await prepareComposeBox(
        ...
        messages: [
          eg.dmMessage(from: user, to: [eg.selfUser]),
        ],
      );
      check(controller).isNotNull().contentFocusNode.hasFocus.isFalse();
    });
  ...
  group('ComposeContentController', () {
    group('insertPadded', () {
      ...
      testInsertPadded('empty; insert one line', '^', 'a\n', 'a\n\n^');
      testInsertPadded('empty; insert two lines', '^', 'a\nb\n', 'a\nb\n\n^');

      group('insert at end', () {
        testInsertPadded(
          'one empty line; insert one line',
          '\n^',
          'a\n',
          '\na\n\n^',
        );
        testInsertPadded(
          'two empty lines; insert one line',
          '\n\n^',
          'a\n',
          '\n\na\n\n^',
        );
        testInsertPadded(
          'one line, incomplete; insert one line',
          'a^',
          'b\n',
          'a\n\nb\n\n^',
        );
        testInsertPadded(
          'one line, complete; insert one line',
          'a\n^',
          'b\n',
          'a\n\nb\n\n^',
        );
        testInsertPadded(
          'multiple lines, last is incomplete; insert one line',
          'a\nb^',
          'c\n',
          'a\nb\n\nc\n\n^',
        );
        testInsertPadded(
          'multiple lines, last is complete; insert one line',
          'a\nb\n^',
          'c\n',
          'a\nb\n\nc\n\n^',
        );
        testInsertPadded(
          'multiple lines, last is complete; insert two lines',
          'a\nb\n^',
          'c\nd\n',
          'a\nb\n\nc\nd\n\n^',
        );
      ...
      group('insert in middle', () {
        testInsertPadded('middle of line', 'a^a\n', 'b\n', 'a\n\nb\n\n^a\n');
        testInsertPadded(
          'start of non-empty line, after empty line',
          'b\n\n^a\n',
          'c\n',
          'b\n\nc\n\n^a\n',
        );
        testInsertPadded(
          'end of non-empty line, before non-empty line',
          'a^\nb\n',
          'c\n',
          'a\n\nc\n\n^b\n',
        );
        testInsertPadded(
          'start of non-empty line, after non-empty line',
          'a\n^b\n',
          'c\n',
          'a\n\nc\n\n^b\n',
        );
    ...
    group('ContentValidationError.empty', () {
      ...
      void checkCountsAsEmpty(String text, bool expected) {
        ...
        expected
            ? check(
                controller,
              ).validationErrors.contains(ContentValidationError.empty)
            : check(controller).validationErrors.not(
                (it) => it.contains(ContentValidationError.empty),
              );
      }
  ...
  group('length validation', () {
    ...
    String makeStringWithCodePoints(int n) {
      ...
      final result = graphemeCluster * (n ~/ 5) + 'a' * (n % 5);
    ...
    group('content', () {
      Future<void> prepareWithContent(
        WidgetTester tester,
        String content,
      ) async {
    ...
    group('topic', () {
      ...
      Future<void> checkErrorResponse(
        WidgetTester tester, {
        required int maxTopicLength,
      }) async {
        await tester.tap(
          find.byWidget(
            checkErrorDialog(
              ...
              expectedTitle: 'Message not sent',
              expectedMessage:
                  'Topic length shouldn\'t be greater than $maxTopicLength ${maxTopicLength == 1 ? 'character' : 'characters'}.',
            ),
          ),
        );
      }
      ...
      testWidgets('too-long topic is rejected', (tester) async {
        await prepareWithTopic(
          tester,
          makeStringWithCodePoints(37 + 1),
          maxTopicLength: 37,
        );
        await tapSendButton(tester);
      ...
      testWidgets('code points not counted unnecessarily', (tester) async {
        ...
        check(
          (controller as StreamComposeBoxController)
              .topic
              .debugLengthUnicodeCodePointsIfLong,
        ).isNull();
      });
    });
  });
  ...
  group('ComposeBox hintText', () {
    ...
    void checkComposeBoxHintTexts(
      ...
      if (topicHintText != null) {
        check(
          tester.widget<TextField>(topicInputFinder),
    ...
    group('to ChannelNarrow, topics not mandatory', () {
      ...
      testWidgets('legacy: with empty topic, topic input has focus', (
        tester,
      ) async {
        await prepare(
          tester,
      ...
      testWidgets('with non-empty but vacuous topic, topic input has focus', (
        tester,
      ) async {
        await prepare(tester, narrow: narrow, mandatoryTopics: false);
        await enterTopic(
      ...
      testWidgets(
        'with empty topic, topic input has focus, then content input gains focus',
        (tester) async {
          await prepare(tester, narrow: narrow, mandatoryTopics: false);
          await enterTopic(tester, narrow: narrow, topic: '');
          await tester.pump();
      ...
      testWidgets('with empty topic, topic input has focus, then loses it', (
        tester,
      ) async {
        await prepare(tester, narrow: narrow, mandatoryTopics: false);
        await enterTopic(tester, narrow: narrow, topic: '');
      ...
      testWidgets('with empty topic, content input has focus', (tester) async {
        ...
        check(tester.widget<TextField>(topicInputFinder)).decoration
            .isNotNull()
            .hintStyle
            .isNotNull()
            .fontStyle
      ...
      testWidgets('legacy: with empty topic, content input has focus', (
        tester,
      ) async {
        await prepare(
          tester,
        ...
        check(
          tester.widget<TextField>(topicInputFinder),
        ).decoration.isNotNull().hintStyle.isNotNull().fontStyle.isNull();
      });

      testWidgets(
        'with empty topic, content input has focus, then topic input gains focus',
        (tester) async {
          await prepare(tester, narrow: narrow, mandatoryTopics: false);
          await enterContent(tester, '');
          await tester.pump();
      ...
      testWidgets('with empty topic, content input has focus, then loses it', (
        tester,
      ) async {
        await prepare(tester, narrow: narrow, mandatoryTopics: false);
        await enterContent(tester, '');
    ...
    testWidgets('to group DmNarrow', (tester) async {
      await prepare(
        tester,
        narrow: DmNarrow.withOtherUsers([
          eg.otherUser.userId,
          eg.thirdUser.userId,
        ], selfUserId: eg.selfUser.userId),
      );
      checkComposeBoxHintTexts(tester, contentHintText: 'Message group');
    });
  });

  group('ComposeBox textCapitalization', () {
    void checkComposeBoxTextFields(
      WidgetTester tester, {
      required bool expectTopicTextField,
    }) {
      if (expectTopicTextField) {
        final topicController =
            (controller as StreamComposeBoxController).topic;
        ...
        check(
          topicTextField,
        ).isNotNull().textCapitalization.equals(TextCapitalization.none);
      } else {
      ...
      final contentTextField = tester.widget<TextField>(
        find.byWidgetPredicate(
          (widget) =>
              widget is TextField && widget.controller == controller!.content,
        ),
      );
      check(
        contentTextField,
  ...
  group('ComposeBox typing notices', () {
    ...
    Future<void> checkStartTyping(
      WidgetTester tester,
      SendableNarrow narrow,
    ) async {
    ...
    testWidgets('clearing text sends a "typing stopped" notice', (
      tester,
    ) async {
      await prepareComposeBox(
        tester,
    ...
    testWidgets('hitting send button sends a "typing stopped" notice', (
      tester,
    ) async {
      MessageStoreImpl.debugOutboxEnable = false;
      addTearDown(MessageStoreImpl.debugReset);
    ...
    testWidgets('navigating away sends a "typing stopped" notice', (
      tester,
    ) async {
    ...
    testWidgets('selection change sends a "typing started" notice', (
      tester,
    ) async {
      await prepareComposeBox(
        tester,
      ...
      connection.prepare(json: {});
      controller!.content.selection = const TextSelection(
        baseOffset: 0,
        extentOffset: 2,
      );
      checkTypingRequest(TypingOp.start, narrow);
    ...
    testWidgets('unfocusing app sends a "typing stopped" notice', (
      tester,
    ) async {
      await prepareComposeBox(
        tester,
  ...
  group('message-send request response', () {
    ...
    testWidgets('success', (tester) async {
      await setupAndTapSend(
        tester,
        prepareResponse: (int messageId) {
    ...
    testWidgets('if channel is unsubscribed, refresh on message-send success', (
      tester,
    ) async {
      ...
      connection.prepare(
        json: SendMessageResult(id: 456).toJson(),
  ...
  group('sending to empty topic', () {
    ...
    testWidgets('empty topic -> ""', (tester) async {
      await setupAndTapSend(tester, topicInputText: '', mandatoryTopics: false);
      check(connection.lastRequest).isA<http.Request>()
        ..method.equals('POST')
        ..url.path.equals('/api/v1/messages')
    ...
    testWidgets('if topics are mandatory, reject empty topic', (tester) async {
      await setupAndTapSend(tester, topicInputText: '', mandatoryTopics: true);
      checkMessageNotSent(tester);
    });

    testWidgets(
      'if topics are mandatory, reject `realmEmptyTopicDisplayName`',
      (tester) async {
        await setupAndTapSend(
          tester,
    ...
    group('add video call link', () {
      ...
      testWidgets('zoom success with token', (tester) async {
        await prepare(
          ...
          realmAvailableVideoChatProvider: {
            'zoom': RealmAvailableVideoChatProviders(name: 'zoom', id: 3),
          },
        );
        ...
        check(controller!.content.text).equals(
          '[Join video call.](https://zoom.us/j/1234567890?pwd=abcdef)\n\n',
        );
      });
    });
  });
  ...
  group('uploads', () {
    void checkAppearsLoading(WidgetTester tester, bool expected) {
      final sendButtonElement = tester.element(
        find.ancestor(of: sendButtonFinder, matching: find.byType(IconButton)),
      );
      ...
      testWidgets('success', (tester) async {
        ...
        testBinding.pickFilesResult = FilePickerResult([
          PlatformFile(
            ...
            path:
                '/private/var/mobile/Containers/Data/Application/foo/tmp/image.jpg',
            name: 'image.jpg',
            size: 12345,
          ),
        ]);
        connection.prepare(
          ...
          json: UploadFileResult(
            url: '/user_uploads/1/4e/m2A3MSqFnWRLUf9SaPzQ0Up_/image.jpg',
          ).toJson(),
        );
    ...
    testWidgets('use verbatim URL string from server, not re-encoded', (
      tester,
    ) async {
      ...
      connection.prepare(
        json: UploadFileResult(
          url: '/user_uploads/1/4e/m2A3MSqFnWRLUf9SaPzQ0Up_/한국어 파일.txt',
        ).toJson(),
      );
      await tester.tap(find.byIcon(ZulipIcons.image));
      await tester.pump();
      check(controller!.content.text).equals('[Uploading 한국어 파일.txt…]()\n\n');
    ...
    group('attach from keyboard', () {
      ...
      Future<void> insertContentFromKeyboard(
        WidgetTester tester, {
        ...
        await tester.binding.defaultBinaryMessenger.handlePlatformMessage(
          ...
          SystemChannels.textInput.codec.encodeMethodCall(
            MethodCall('TextInputClient.performAction', <dynamic>[
              ...
              {"mimeType": mimeType, "data": data, "uri": attachedFileUrl},
            ]),
          ),
          (ByteData? data) {},
        );
      ...
      testWidgets('success', (tester) async {
        ...
        await insertContentFromKeyboard(
          tester,
          data: fileContent,
          attachedFileUrl:
  ...
  group('error banner', () {
    ...
    void checkComposeBoxParts({required bool areShown}) {
      ...
      areShown
          ? check(inputFieldCount).isGreaterThan(0)
          : check(inputFieldCount).equals(0);
      check(
        attachButtonFinder(ZulipIcons.attach_file).evaluate().length,
      ).equals(areShown ? 1 : 0);
      check(
        attachButtonFinder(ZulipIcons.image).evaluate().length,
      ).equals(areShown ? 1 : 0);
      check(
        attachButtonFinder(ZulipIcons.camera).evaluate().length,
      ).equals(areShown ? 1 : 0);
      check(
        attachButtonFinder(ZulipIcons.video).evaluate().length,
      ).equals(areShown ? 1 : 0);
    }
    ...
    group('in DMs with deactivated users', () {
      void checkComposeBox({required bool isShown}) => checkComposeBoxIsShown(
        isShown,
      ...
      Future<void> changeUserStatus(
        ...
        await store.handleEvent(
          RealmUserUpdateEvent(id: 1, userId: user.userId, isActive: isActive),
        );
        await tester.pump();
      }

      DmNarrow dmNarrowWith(User otherUser) =>
          DmNarrow.withUser(otherUser.userId, selfUserId: eg.selfUser.userId);

      DmNarrow groupDmNarrowWith(List<User> otherUsers) =>
      ...
      group('1:1 DMs', () {
        testWidgets('compose box replaced with a banner', (tester) async {
          ...
          await prepareComposeBox(
            tester,
            narrow: dmNarrowWith(deactivatedUser),
        ...
        testWidgets('active user becomes deactivated -> '
          ...
          await prepareComposeBox(
            tester,
            narrow: dmNarrowWith(activeUser),
      ...
      group('group DMs', () {
        testWidgets('compose box replaced with a banner', (tester) async {
          final deactivatedUsers = [
            eg.user(isActive: false),
            eg.user(isActive: false),
          ];
          await prepareComposeBox(
            tester,
            narrow: groupDmNarrowWith(deactivatedUsers),
        ...
        testWidgets('at least one user becomes deactivated -> '
            'compose box is replaced with a banner', (tester) async {
          final activeUsers = [
            eg.user(isActive: true),
            eg.user(isActive: true),
          ];
          await prepareComposeBox(
            tester,
            narrow: groupDmNarrowWith(activeUsers),
    ...
    group('in channel/topic narrow according to channel post policy', () {
      ...
      void testRefreshSubscribeButtons({required Narrow narrow}) {
        testWidgets(
          'Refresh/Subscribe buttons when cannot send and channel unsubscribed, $narrow',
          (tester) async {
            final channel = eg.stream(
              streamId: 1,
            ...
            final messages = List.generate(
              ...
              (i) => eg.streamMessage(
                id: 1000 + i,
                stream: channel,
                topic: topicNarrow.topic.apiName,
              ),
            );
            ...
            checkComposeBoxIsShown(
              false,
              bannerLabel: zulipLocalizations
                  .composeBoxBannerLabelUnsubscribedWhenCannotSend,
            );
            ...
            await tester.tap(
              find.widgetWithText(ZulipWebUiKitButton, 'Refresh'),
            );
            await tester.pump();
            check(model)
              ..fetched.isFalse()
              ..messages.length.equals(0);
            ...
            await tester.tap(
              find.widgetWithText(ZulipWebUiKitButton, 'Subscribe'),
            );
            await tester.pump();
            await tester.pump(Duration.zero);
            check(connection.lastRequest).isA<http.Request>()
      ...
      testWidgets(
        'user loses privilege -> compose box is replaced with the banner',
        (tester) async {
      ...
      testWidgets(
        'user gains privilege -> banner is replaced with the compose box',
        (tester) async {
      ...
      testWidgets(
        'channel policy becomes stricter -> compose box is replaced with the banner',
        (tester) async {
          final selfUser = eg.user(role: UserRole.guest);
          final channel = eg.stream(
            streamId: 1,
      ...
      testWidgets(
        'channel policy becomes less strict -> banner is replaced with the compose box',
        (tester) async {
  ...
  group('ComposeBox content input scaling', () {
    ...
    Future<void> checkContentInputMaxHeight(
      WidgetTester tester, {
    ...
    testWidgets('normal text scale factor', (tester) async {
      ...
      await checkContentInputMaxHeight(
        tester,
        maxHeight: verticalPadding + 170,
        maxVisibleLines: 8,
      );
    });
    ...
    testWidgets('lower text scale factor', (tester) async {
      ...
      await checkContentInputMaxHeight(
        tester,
        maxHeight: verticalPadding + 170 * 0.8,
        maxVisibleLines: 8,
      );
    });
    ...
    testWidgets('higher text scale factor', (tester) async {
      ...
      await checkContentInputMaxHeight(
        tester,
        maxHeight: verticalPadding + 170 * 1.5,
        maxVisibleLines: 8,
      );
    });
    ...
    testWidgets('content input not cleared when store changes', (tester) async {
      ...
      final newConnection =
          (testBinding.globalStore
                  ...
                  .apiConnectionFromAccount(store.account)
  ...
  Future<void> startEditInteractionFromActionSheet(
    ...
    await tester.longPress(
      find.byWidgetPredicate(
        (widget) =>
            widget is MessageWithPossibleSender &&
            widget.item.message.id == messageId,
      ),
    );
  ...
  Future<void> expectAndHandleDiscardConfirmation(
    ...
    final (actionButton, cancelButton) = checkSuggestedActionDialog(
      tester,
  ...
  group('restoreMessageNotSent', () {
    ...
    final failedMessageFinder = find.widgetWithText(
      OutboxMessageWithPossibleSender,
      failedMessageContent,
      skipOffstage: true,
    );

    Future<void> prepareMessageNotSent(
      WidgetTester tester, {
      required Narrow narrow,
      List<User> otherUsers = const [],
    }) async {
    ...
    Future<void> expectAndHandleDiscardForMessageNotSentConfirmation(
      ...
      return expectAndHandleDiscardConfirmation(
        tester,
        expectedMessage:
    ...
    testWidgets(
      'interrupting new-message compose: proceed through confirmation dialog',
      (tester) async {
        await prepareMessageNotSent(tester, narrow: topicNarrow);
        ...
        await expectAndHandleDiscardForMessageNotSentConfirmation(
          tester,
          shouldContinue: true,
        );
        await tester.pump();
    ...
    testWidgets(
      'interrupting new-message compose: cancel confirmation dialog',
      (tester) async {
        await prepareMessageNotSent(tester, narrow: topicNarrow);
        await enterContent(tester, 'composing something');
    ...
    testWidgets(
      'interrupting message edit: proceed through confirmation dialog',
      (tester) async {
        await prepareMessageNotSent(tester, narrow: topicNarrow);
        ...
        await startEditInteractionFromActionSheet(
          tester,
    ...
    testWidgets('interrupting message edit: cancel confirmation dialog', (
      tester,
    ) async {
      await prepareMessageNotSent(tester, narrow: topicNarrow);
  ...
  group('edit message', () {
    ...
    Future<void> prepareEditMessage(
      WidgetTester tester, {
      required Narrow narrow,
    }) async {
      MessageStoreImpl.debugOutboxEnable = false;
    ...
    Future<void> checkAwaitingRawMessageContent(WidgetTester tester) async {
      ...
      check(
        tester.widget(contentInputFinder),
      ).isA<TextField>().decoration.isNotNull().hintText.equals('Preparing…');
      checkContentInputValue(tester, '');

      // Controls are disabled
      await tester.tap(
        find.byIcon(ZulipIcons.attach_file),
        warnIfMissed: false,
      );
      await tester.pump();
    ...
    void checkRequest(
      ...
      final prevContentSha256 = sha256
          .convert(utf8.encode(prevContent))
          .toString();
      check(connection.takeRequests()).single.isA<http.Request>()
        ..method.equals('PATCH')
    ...
    void checkEditInProgressInMsglist(
      WidgetTester tester, {
      required int messageId,
      required bool expected,
    }) {
      ...
      check(
        find
            .descendant(of: messageFinder, matching: find.text('SAVING EDIT…'))
            .evaluate()
            .length,
      ).equals(expected ? 1 : 0);
    }
    ...
    void testSmoke({
      required Narrow narrow,
      required _EditInteractionStart start,
    }) {
      testWidgets('smoke: $narrow, ${start.message()}', (tester) async {
        ...
        connection.prepare(
          json: UploadFileResult(url: '/path/file.jpg').toJson(),
        );
        await tester.tap(
          find.byIcon(ZulipIcons.attach_file),
          warnIfMissed: false,
        ...
        checkRequest(
          messageId,
          prevContent: 'foo',
          content: 'some new content[file.jpg](/path/file.jpg)',
        );
        await tester.pump(Duration.zero);
        checkNotInEditingMode(tester, narrow: narrow);
    ...
    Future<void> expectAndHandleDiscardForEditConfirmation(
      WidgetTester tester, {
      required bool shouldContinue,
    }) {
      return expectAndHandleDiscardConfirmation(
    ...
    void testInterruptComposingFromActionSheet({required Narrow narrow}) {
      testWidgets('interrupting new-message compose: $narrow', (tester) async {
        ...
        await expectAndHandleDiscardForEditConfirmation(
          tester,
          shouldContinue: false,
        );
        check(connection.takeRequests()).isEmpty();
        ...
        checkNotInEditingMode(
          tester,
          narrow: narrow,
          expectedContentText: 'composing new message',
        );
        await enterContent(tester, 'composing new message…');
    ...
    void testInterruptComposingFromFailedEdit({required Narrow narrow}) {
      testWidgets(
        'interrupting new-message compose by tapping failed edit to restore: $narrow',
        (tester) async {
          TypingNotifier.debugEnable = false;
          addTearDown(TypingNotifier.debugReset);
    ...
    void testCancel({
      ...
      testWidgets(description.toString(), (tester) async {
        ...
        if (start == _EditInteractionStart.actionSheet &&
            duringFetchRawContentRequest!) {
    ...
    testWidgets('if channel is unsubscribed, refresh on message-edit success', (
      tester,
    ) async {
      // Regression test for the "first buggy behavior"
      // in https://github.com/zulip/zulip-flutter/issues/1798 .
      ...
      final updatedMessage = Message.fromJson(
        message.toJson()..['content'] = ContentExample.emojiUnicode.html,
      );
      connection.prepare(
        json: eg
            .newestGetMessagesResult(
